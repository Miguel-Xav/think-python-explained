...
def triangle_center(char, n): #same basis as the previous function
    for i in range(n): #same loop as before
        ...
        print(" " * space + (char * (i)) + char+(char*i)) # same as before the exercise 2, but now we add char+(char*i) to make the other side (the triangle go 1,3,6...)


triangle_center("L", 5)

#I have created this just as a challenge for myself. I think is a good way to fix the thing that we have learned in the previous exercises and also to practice with loops and charing manipulation. 
